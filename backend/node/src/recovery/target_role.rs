...
type RecoverKeyshareResult<C> = Result<(Scalar<C>, Vec<VerifiableSS<C>>, Point<C>)>;
    ...
    where C: Curve
{
    ...
    RecoveryCalculator::<C>::validate_recovered_share(&secret, &vss, recovery_index)?;
...
fn validate_all_matching_items<T: PartialEq + Clone>(items: &[T], description: &str) -> Result<T> {
    ...
    let all_matching = items.iter().all(|item| item == first_item);
    if !all_matching {
