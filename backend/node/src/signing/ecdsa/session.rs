...
impl SignSession {
    ...
    #[instrument(skip_all)]
    fn phase1(&self, signers_vec: &[usize]) -> anyhow::Result<Phase1Data> {
        let g_w_vec = SignKeys::g_w_vec(
            &self.keyshare.public_key_vec.iter().cloned().map_into().collect::<Vec<_>>(),
            signers_vec,
            &self.keyshare.vss_scheme_vec[self.keyshare.party_index - 1].clone()
        );
    ...
    #[instrument(skip_all)]
    fn phase2(&self, signers_vec: &[usize], p1d: &Phase1Data) -> anyhow::Result<Phase2Data> {
        ...
        for i in 0..THRESHOLD {
            if i != self.party_info.id_in_session {
                ...
                let g_w_i = Keys::update_commitments_to_xi(
                    ...
                    &self.keyshare.vss_scheme_vec[signers_vec[i]].clone(),
                    signers_vec[i],
                    signers_vec
                );
    ...
    fn phase5(
        ...
        for i in 0..THRESHOLD {
            ...
            let proof = LocalSignature::phase5_proof_pdl(
                ...
                &p1d.sign_keys.k_i,
                &p1d.randomness,
                &self.keyshare.h1_h2_N_tilde_vec[signers_vec[i]].clone() // changed from dlog_statement_vec
            );
