...
impl ConfigProvider for ConfigGridlock {
    ...
    fn get_nats_address() -> String {
        match std::env::var("NATS_NETWORK") {
            Ok(addr) if !addr.is_empty() => {
                println!("Using NATS server address from environment: {}", addr);
                addr
            }
            _ => {
                // If NATS_NETWORK is missing or empty in that file, it's a critical configuration error.
                panic!(
                    "NATS_NETWORK environment variable is not set or is empty. Please ensure it is defined in your .env file."
                );
            }
        }
    }
